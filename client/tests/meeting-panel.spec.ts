...
test.describe('Meeting panel', () => {
  ...
  test('creates a meeting session and shows transcript details', async ({ page }) => {
    ...
    await page.route('**/meeting/sessions/*/summarize', async (route) => {
    ...
    await Promise.all([
      page.waitForResponse(
        (res) => res.url().includes('/meeting/sessions') && res.request().method() === 'POST'
      ),
    ...
    await Promise.all([
      page.waitForResponse(
        (res) => res.url().includes('/meeting/sessions/') && res.request().method() === 'GET'
      ),
      sessionCard.click()
    ])
