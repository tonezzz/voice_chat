...
logger = logging.getLogger("mcp_seqthink")
...
class SequentialThinkingBridge:
    def __init__(self, binary: Optional[str] = None) -> None:
        self._binary = binary or os.environ.get("SEQTHINK_BINARY", "mcp-server-sequential-thinking")
    ...
    async def start(self) -> None:
        ...
        logger.info("Starting sequential thinking MCP bridge")
    ...
    async def stop(self) -> None:
        if not self._proc:
            return
        logger.info("Stopping sequential thinking MCP bridge")
    ...
    async def _initialize_session(self) -> None:
        response = await self._request(
            "initialize",
            {
                ...
                "capabilities": {},
                "clientInfo": {"name": "mcp-seqthink-bridge", "version": "0.1.0"},
            },
        )
    ...
    async def _request(self, method: str, params: Dict[str, Any]) -> Dict[str, Any]:
        if not self.is_running or not self._writer:
            raise HTTPException(status.HTTP_503_SERVICE_UNAVAILABLE, "sequential thinking bridge not running")
    ...
    async def _listen_for_responses(self) -> None:
        ...
        while True:
            ...
            try:
                ...
                logger.warning("Failed to decode sequential thinking message: %s", line)
                continue
    ...
    async def _drain_stream(self, stream: asyncio.StreamReader, label: str) -> None:
        while True:
            ...
            if not data:
                break
            logger.info("seqthink %s: %s", label, data.decode().rstrip())
...
bridge = SequentialThinkingBridge()

PORT = int(os.environ.get("PORT", "8015"))
PROVIDER_NAME = os.environ.get("SEQTHINK_PROVIDER_NAME", "seqthink")
DEFAULT_TOOLS = [
    tool.strip()
    for tool in os.environ.get("SEQTHINK_DEFAULT_TOOLS", "sequentialthinking").split(",")
    if tool.strip()
]
MCP0_URL = os.environ.get("MCP0_URL")
MCP0_ADMIN_TOKEN = os.environ.get("MCP0_ADMIN_TOKEN")
SEQTHINK_BASE_URL = os.environ.get("SEQTHINK_BASE_URL", f"http://mcp-seqthink:{PORT}")
...
app = FastAPI(title="mcp-seqthink-bridge", version="0.1.0", lifespan=lifespan)
